...
class ApiService {
  ...
  private async makeRequest<T>(
    ...
    try {
      ...
      if (!usesApiKey) {
        ...
        if (!privyToken) {
          console.error("Authentication unavailable - Privy token required");
          return {
            success: false,
  ...
  async getProjectContractConfig(projectId: string): Promise<ApiResponse<{ subscription_manager_address: string; token_address: string }>> {
    const response = await this.makeRequest<{
      success?: boolean;
      data?: {
        project?: {
          subscription_manager_address?: string;
          supported_token?: { token_address?: string | null } | null;
        };
      };
      error?: string;
    }>(
    ...
    if (!response.success) {
      return {
        success: false,
        error: response.error || "Failed to load project details",
      };
    }
    ...
    const subscriptionManagerAddress = payload?.data?.project?.subscription_manager_address;
    const tokenAddress = payload?.data?.project?.supported_token?.token_address ?? null;

    if (!subscriptionManagerAddress || !tokenAddress) {
      return {
        success: false,
        error: "Project is missing contract configuration",
      };
    }
